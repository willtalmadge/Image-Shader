...
#include "FFTInterleaveToFt.h"

const std::string FFTInterleaveToFt::fragShaderRe = SHADER_STRING
(
 ...
 void main()
{
    ...
    highp vec4 result;
    result.rgb = 0.5*((im.rgb+imn2.rgb)*p.x + re.rgb + p.y*re.rgb + ren2.rgb - p.y*ren2.rgb);
    result.a = 1.0;
    gl_FragColor = result;
}
 );
const std::string FFTInterleaveToFt::fragShaderIm = SHADER_STRING
(
 varying highp vec2 n;
 varying highp vec2 N2mn;
 ...
 void main()
{
    ...
    highp vec4 p = texture2D(phaseTable, n);
    highp vec4 result;
    result.rgb = 0.5*(im.rgb - imn2.rgb + (im.rgb + imn2.rgb)*p.y + p.x*(ren2.rgb-re.rgb));
    result.a = 1.0;
    gl_FragColor = result;
}
 );

const std::string FFTInterleaveToFt::vertShader = SHADER_STRING
(
